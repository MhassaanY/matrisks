...
Matrisks Framework is an Android vulnerability analysis system that helps developers or hackers find potential security vulnerabilities in Android applications. 
...
## Setup
The following requirements are needed in order to fully use Matrisks Framework's features:
- Python 3
...
Optionally reconfigure the MongoDB config in `matrisks-db.cfg` to match your MongoDB configuration. The database will be created if it does not exists already.

## Usage for Unix/Linux

### Running the Matrisks Framework
To scan an application for all the defined vulnerabilities run the following command:
```
python matrisks.py -f [APK file]
```
Optionally, you can specify which vectors you would like to scan, using 
```
python matrisks.py -f [APK file] -d [Vector Name]
```
...
```
python matrisks.py -l
```

### Getting help for parameters

```
python matrisks.py -h
```

### Usage of Massive Analysis Tools for Unix/Linux

```
python Matrisks_MassiveAnalysis.py -b [Analysis Date] -t [Anaysis Tag] -d [APKs input directory] -o [Report output directory]
```
 
Example:
```
python Matrisks_MassiveAnalysis.py -b 30072020 -t BlackHat -d ~/APKDataset/ -o ~/Massive_Analysis_Reports
```
...
```
python Matrisks_ReportSummary.py -m massive -b [Analysis Date] -t [Anaysis Tag]
```

Example:
```
python Matrisks_ReportSummary.py -m massive -b 30072020 -t BlackHat
```
...
```
python Matrisks_ReportByVectorKey.py -v [Vector ID] -l [Log Level] -b [Analysis Date] -t [Anaysis Tag]
python Matrisks_ReportByVectorKey.py -v [Vector ID] -l [Log Level] -b [Analysis Date] -t [Anaysis Tag] -a
```

Example:
```
python Matrisks_ReportByVectorKey.py -v WEBVIEW_RCE -l Critical -b 30072020 -t BlackHat
python Matrisks_ReportByVectorKey.py -v WEBVIEW_RCE -l Critical -b 30072020 -t BlackHat -a
```

![Matrisks_ReportSummary.py](http://www.matrisks.com/images/v1.0.0/MassiveAnalysisTool2.png)
...
* Matrisks Framework is under the license of [GNU GPL v3.0](http://www.gnu.org/licenses/gpl-3.0.txt)
