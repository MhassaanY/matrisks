...
def parse_vulnerabilities(report_content):
    vulnerabilities = []
    # Regex to find the start of a vulnerability block and capture severity, category, title and vector name.
    vulnerability_pattern = re.compile(r"^\[([A-Za-z]+)\](?:\s*<([^>]+)>)?\s*(.*?)(?:\s*\(Vector: (.*?)\))?:", re.MULTILINE)
    ...
    while line_idx < len(lines):
        ...
        if match:
            ...
            category = match.group(2).strip() if match.group(2) else match.group(4).strip() if match.group(4) else "General"
            title = match.group(3).strip()
            vector = match.group(4).strip() if match.group(4) else "Unknown"
        else:
            line_idx += 1
            continue
...
def generate_html_report(metadata, vulnerabilities, raw_report_text, output_path, template_path="dashboard_template.html"):
    ...
    for v in vulnerabilities:
        ...
        vuln_rows_html += f'<td>{v["vector"]}</td>'
        vuln_rows_html += '</tr>'
        vuln_rows_html += '<tr class="details-row"><td colspan="5">'
    ...
    vector_data_json = json.dumps(vector_counts)

    # 5. Create Findings by Vector Table
    vector_table_html = '<table class="vector-table">'
    vector_table_html += '<tr><th>Vector</th><th>Count</th></tr>'
    for vector, count in vector_counts.items():
        vector_table_html += f'<tr onclick="filterByVector(\'{vector}\')"><td>{vector}</td><td>{count}</td></tr>'
    vector_table_html += '</table>'
    ...
    template = template.replace('{{VECTOR_DATA_JSON}}', vector_data_json)
    template = template.replace('{{VECTOR_TABLE}}', vector_table_html)
