...
def __analyze(writer, args, config):
    ...
    for vector_name in enabled_vectors:
        try:
            module = importlib.import_module(f'vectors.{vector_name}')
            loaded_vector_classes.append(module)
            print(vector_name)
        except ImportError:
    ...
    for vector_class in loaded_vector_classes:
        if args.debug_vector is None or args.debug_vector in vector_class.Vector.tags:
            ...
            vector_class.Vector(writer, a, d, dx, vector_class.__name__, args, int_min_sdk, int_target_sdk, config).analyze()
