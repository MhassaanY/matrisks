...
class Writer:
    ...
    def save_result_to_file(self, args):
        if not self.__file_io_result_output_list:
            self.load_to_output_list(args)

        output_file_path = os.path.join(self.staging_dir, "report-raw.txt")
    ...
    def get_full_report_content(self, args):
        if not self.__file_io_result_output_list:
            self.load_to_output_list(args)
    ...
    def save_csv_report(self, output_path):
        """
        Saves the complete analysis result as a CSV file.
        """
        self.completeWriter()  # Ensure all cached data is written to the main dictionary
        try:
            with open(output_path, 'w', newline='') as f:
                ...
                # Header row
                csv_writer.writerow(["Severity", "Category", "Title", "Summary", "Suggestion", "Risk", "Confidence", "Vector", "Details"])

                for tag, dict_information in self.__output_dict_vector_result_information.items():
                    severity = dict_information.get("level", "N/A")
                    category = ", ".join(dict_information.get("special_tag", [])) if dict_information.get("special_tag") else "N/A"
                    title = dict_information.get("title", "N/A")
                    summary = dict_information.get("summary", "N/A")
                    suggestion = dict_information.get("suggestion", "N/A")
                    risk = dict_information.get("risk", "N/A")
                    confidence = dict_information.get("confidence", "N/A")
                    vector_name = dict_information.get("vector_name", "N/A")
                    details = dict_information.get("vector_details", "N/A")

                    csv_writer.writerow([severity, category, title, summary, suggestion, risk, confidence, vector_name, details])
            print(f"<<< CSV report is generated: {os.path.abspath(output_path)} >>>")
        except IOError as err:
            print(f"[Error on writing CSV output file to disk: {err}]")
